...
import EmployeeForm from "./Form01";

const ToolRequestsTable = () => {
  ...
  const changeStatus = async () => {
    if (recordTOChangeStatus) {
      const requestDoc = doc(db, "ToolRequests", recordTOChangeStatus.id);
    ...
    <div className="p-2 md:p-4 mt-20 w-full h-full   flex flex-col items-center justify-center    md:place-self-center  text-navy_blue">
      <div className="w-full bg-white md:m-6 rounded-md p-4 md:p-8 ">
            ...
            <tbody>
              {visibleData.map((request, index) => (
                <tr key={index} className="hover:bg-gray-100 even:bg-gray-200/20">
                  ...
                  {/* <td className="px-4 py-2 border">{request.createdAt}</td> */}
                  <td className="px-4 py-2 border">
                    <button
                      onClick={() => openPopup(request.imageUrl)}
                      className="flex items-center text-blue-500 hover:underline mr-2"
                    >
                      <FiImage className="mr-1" /> View Image
                    </button>
        ...
        <Popup
          ...
          onClose={closePopup}
        >
          <div className="border border-black shadow bg-gray-700 m-0  relative text-white rounded-lg">
            <button
              type="button"
              className="absolute top-3 right-2.5 text-black bg-white hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 inline-flex justify-center items-center"
            ...
            <img src={popupImage} alt="Popup" className="max-w-full max-h-96" />
          </div>
        </Popup>
